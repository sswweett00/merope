...
func (s *Scheduler) Register(name string, interval time.Duration, task TaskFunc) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	s.tasks = append(s.tasks, &ScheduledTask{Name: name, Interval: interval, Task: task})
}
...
func (c *CronRunner) Register(name, cronExpr string, task TaskFunc) {
	c.jobs = append(c.jobs, &CronJob{Name: name, CronExpr: cronExpr, Task: task})
}
