...
func (s *mediaService) TransformMedia(ctx context.Context, mediaID string, transformation *domain.MediaTransformation) (*domain.UploadResult, error) {
	...
	targetWidth := transformation.Width; if targetWidth <= 0 { targetWidth = 800 }
	quality := transformation.Quality; if quality <= 0 { quality = 85 }
...
func (s *mediaService) GenerateThumbnail(ctx context.Context, mediaID string) (*domain.UploadResult, error) {
	metadata, err := s.repo.GetMediaMetadata(ctx, mediaID); if err != nil { return nil, err }
	reader, err := s.s3Client.Download(ctx, metadata.StorageKey); if err != nil { return nil, err }; defer reader.Close()
	img, _, err := image.Decode(reader); if err != nil { return nil, err }
	thumbBuf, err := s.createScaledImageBuffer(img, 250, 80); if err != nil { return nil, err }
	...
	metadata.ThumbnailURL = thumbURL; _ = s.repo.UpdateMediaMetadata(ctx, mediaID, metadata)
...
func (s *mediaService) CompressMedia(ctx context.Context, mediaID string, quality int) (*domain.UploadResult, error) {
	...
	metadata, err := s.repo.GetMediaMetadata(ctx, mediaID); if err != nil { return nil, err }
	reader, err := s.s3Client.Download(ctx, metadata.StorageKey); if err != nil { return nil, err }; defer reader.Close()
	img, format, err := image.Decode(reader); if err != nil { return nil, err }
...
func bilinearResize(src image.Image, width, height int) *image.RGBA {
	dst := image.NewRGBA(image.Rect(0, 0, width, height)); sb := src.Bounds(); sw, sh := sb.Dx(), sb.Dy()
	if width <= 0 || height <= 0 || sw <= 0 || sh <= 0 { return dst }
	for y := 0; y < height; y++ {
		gy := 0.0; if height > 1 { gy = float64(y) * float64(sh-1) / float64(height-1) }
		y0 := int(gy); y1 := y0 + 1; if y1 >= sh { y1 = sh - 1 }; fy := gy - float64(y0)
		for x := 0; x < width; x++ {
			gx := 0.0; if width > 1 { gx = float64(x) * float64(sw-1) / float64(width-1) }
			x0 := int(gx); x1 := x0 + 1; if x1 >= sw { x1 = sw - 1 }; fx := gx - float64(x0)
			c00 := color.NRGBAModel.Convert(src.At(sb.Min.X+x0, sb.Min.Y+y0)).(color.NRGBA); c10 := color.NRGBAModel.Convert(src.At(sb.Min.X+x1, sb.Min.Y+y0)).(color.NRGBA)
...
func (s *mediaService) createScaledImageBuffer(src image.Image, targetWidth, quality int) (*bytes.Buffer, error) {
	bounds := src.Bounds(); srcWidth, srcHeight := bounds.Dx(), bounds.Dy(); if srcWidth <= 0 || srcHeight <= 0 { return nil, fmt.Errorf("invalid image dimensions") }
	if targetWidth >= srcWidth { buf := new(bytes.Buffer); if err := jpeg.Encode(buf, src, &jpeg.Options{Quality: quality}); err != nil { return nil, err }; return buf, nil }
	targetHeight := (srcHeight * targetWidth) / srcWidth; if targetHeight < 1 { targetHeight = 1 }
	dst := bilinearResize(src, targetWidth, targetHeight); buf := new(bytes.Buffer); if err := jpeg.Encode(buf, dst, &jpeg.Options{Quality: quality}); err != nil { return nil, err }; return buf, nil
}
...
func (s *mediaService) DeleteMedia(ctx context.Context, mediaID,userID string) error { m,err:=s.repo.GetMediaMetadata(ctx,mediaID); if err!=nil{return err}; if m.UploaderID!=userID{return fmt.Errorf("unauthorized")}; if err:=s.s3Client.Delete(ctx,m.StorageKey); err!=nil{return err}; return s.repo.DeleteMedia(ctx,mediaID) }
func (s *mediaService) UpdateMediaInfo(ctx context.Context, mediaID,altText,description string,tags []string) error { m,err:=s.repo.GetMediaMetadata(ctx,mediaID); if err!=nil{return err}; m.AltText,m.Description,m.Tags,m.UpdatedAt=altText,description,tags,time.Now(); return s.repo.UpdateMediaMetadata(ctx,mediaID,m) }
...
func (s *mediaService) BatchUpload(ctx context.Context,files []domain.UploadRequest,userID string)([]*domain.UploadResult,error){results:=make([]*domain.UploadResult,len(files));for i,file:=range files{var result *domain.UploadResult;var err error;switch file.Category{case "image":result,err=s.UploadImage(ctx,file.Name,file.Body,userID);case "video":result,err=s.UploadVideo(ctx,file.Name,file.Body,userID);case "audio":result,err=s.UploadAudio(ctx,file.Name,file.Body,userID);case "document":result,err=s.UploadDocument(ctx,file.Name,file.Body,userID);default:result,err=s.UploadImage(ctx,file.Name,file.Body,userID)};if err!=nil{return nil,err};results[i]=result};return results,nil}
func (s *mediaService) BatchDelete(ctx context.Context,mediaIDs []string,userID string)error{for _,id:=range mediaIDs{if err:=s.DeleteMedia(ctx,id,userID);err!=nil{return err}};return nil}
func (s *mediaService) SetMediaPublic(ctx context.Context,mediaID string,isPublic bool)error{m,err:=s.repo.GetMediaMetadata(ctx,mediaID);if err!=nil{return err};m.IsPublic,m.UpdatedAt=isPublic,time.Now();return s.repo.UpdateMediaMetadata(ctx,mediaID,m)}
func (s *mediaService) SetMediaExpiration(ctx context.Context,mediaID string,expiresAt time.Time)error{m,err:=s.repo.GetMediaMetadata(ctx,mediaID);if err!=nil{return err};m.ExpiresAt,m.UpdatedAt=&expiresAt,time.Now();return s.repo.UpdateMediaMetadata(ctx,mediaID,m)}
