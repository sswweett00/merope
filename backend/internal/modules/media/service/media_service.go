...
func (s *mediaService) UploadImage(ctx context.Context, name string, body io.Reader, userID string) (*domain.UploadResult, error) {
	...
	if _, err = s.s3Client.Upload(ctx, key, bytes.NewReader(data), mimeType); err != nil {
		return nil, err
	}
...
func (s *mediaService) UploadVideo(ctx context.Context, name string, body io.Reader, userID string) (*domain.UploadResult, error) {
	...
	if _, err = s.s3Client.Upload(ctx, key, bytes.NewReader(data), "video/mp4"); err != nil {
		return nil, err
	}
...
func (s *mediaService) UploadAudio(ctx context.Context, name string, body io.Reader, userID string) (*domain.UploadResult, error) {
	...
	if _, err = s.s3Client.Upload(ctx, key, bytes.NewReader(data), "audio/mpeg"); err != nil {
		return nil, err
	}
...
func (s *mediaService) UploadDocument(ctx context.Context, name string, body io.Reader, userID string) (*domain.UploadResult, error) {
	...
	if _, err = s.s3Client.Upload(ctx, key, bytes.NewReader(data), "application/pdf"); err != nil {
		return nil, err
	}
...
func (s *mediaService) TransformMedia(ctx context.Context, mediaID string, transformation *domain.MediaTransformation) (*domain.UploadResult, error) {
	...
	if _, err = s.s3Client.Upload(ctx, newKey, buf, "image/jpeg"); err != nil {
		return nil, err
	}
...
func (s *mediaService) GenerateThumbnail(ctx context.Context, mediaID string) (*domain.UploadResult, error) {
	...
	if _, err = s.s3Client.Upload(ctx, thumbKey, thumbBuf, "image/jpeg"); err != nil {
		return nil, err
	}
...
func (s *mediaService) CompressMedia(ctx context.Context, mediaID string, quality int) (*domain.UploadResult, error) {
	...
	} else if err = jpeg.Encode(buf, img, &jpeg.Options{Quality: quality}); err != nil {
		return nil, err
	}
	compressedKey := fmt.Sprintf("compressed/%s_q%d.jpg", mediaID, quality)
	if _, err = s.s3Client.Upload(ctx, compressedKey, buf, "image/jpeg"); err != nil {
		return nil, err
	}
...
func bilinearResize(src image.Image, width, height int) *image.RGBA {
	...
	for y := 0; y < height; y++ {
		gy := 0.0
		if height > 1 {
			gy = float64(y) * float64(sh-1) / float64(height-1)
		}
		y0 := int(gy)
		y1 := y0 + 1
		if y1 >= sh {
		...
		for x := 0; x < width; x++ {
			gx := 0.0
			if width > 1 {
				gx = float64(x) * float64(sw-1) / float64(width-1)
			}
			x0 := int(gx)
			x1 := x0 + 1
			if x1 >= sw {
...
func (s *mediaService) GetUserMedia(ctx context.Context, userID string, page int32) ([]*domain.MediaMetadata, error) {
	...
	limit := int32(50)
	return s.repo.GetUserMedia(ctx, userID, limit, page*limit)
}
func (s *mediaService) DeleteMedia(ctx context.Context, mediaID, userID string) error {
	m, err := s.repo.GetMediaMetadata(ctx, mediaID)
	if err != nil {
		return err
	}
	...
	if err := s.s3Client.Delete(ctx, m.StorageKey); err != nil {
		return err
	}
	return s.repo.DeleteMedia(ctx, mediaID)
}
func (s *mediaService) UpdateMediaInfo(ctx context.Context, mediaID, altText, description string, tags []string) error {
	m, err := s.repo.GetMediaMetadata(ctx, mediaID)
	if err != nil {
		return err
	}
	m.AltText, m.Description, m.Tags, m.UpdatedAt = altText, description, tags, time.Now()
	return s.repo.UpdateMediaMetadata(ctx, mediaID, m)
}
func (s *mediaService) SearchMedia(ctx context.Context, query string, page int32) ([]*domain.MediaMetadata, error) {
	...
	limit := int32(50)
	return s.repo.SearchMedia(ctx, query, limit, page*limit)
}
...
func (s *mediaService) SetMediaPublic(ctx context.Context, mediaID string, isPublic bool) error {
	...
	m.IsPublic, m.UpdatedAt = isPublic, time.Now()
	return s.repo.UpdateMediaMetadata(ctx, mediaID, m)
}
func (s *mediaService) SetMediaExpiration(ctx context.Context, mediaID string, expiresAt time.Time) error {
	...
	m.ExpiresAt, m.UpdatedAt = &expiresAt, time.Now()
	return s.repo.UpdateMediaMetadata(ctx, mediaID, m)
}
