...
func (r *PostgresPrivacyRepository) CheckVisibility(ctx context.Context, userID, viewerID string) (bool, error) {
	...
	if err := vid.Scan(viewerID); err != nil {
		return false, fmt.Errorf("invalid viewer id: %w", err)
	}
...
func (r *PostgresPrivacyRepository) UploadOneTimePreKeys(ctx context.Context, userID, deviceID string, keys []domain.OneTimePreKey) error {
	...
	for _, k := range keys {
		if err := r.queries.UpsertOneTimePreKey(ctx, db.UpsertOneTimePreKeyParams{UserID: uid, DeviceID: deviceID, KeyID: k.ID, PublicKey: k.PublicKey}); err != nil {
			return err
		}
	}
	return nil
