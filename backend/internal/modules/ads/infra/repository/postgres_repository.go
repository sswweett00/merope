...
func (r *postgresAdsRepository) GetActiveAds(ctx context.Context) ([]*domain.Ad, error) {
	...
	for rows.Next() {
		ad := &domain.Ad{}
		var interests []byte
		err := rows.Scan(&ad.ID, &ad.CampaignID, &ad.Title, &ad.ContentText, &ad.MediaURL, &interests, &ad.Status)
		if err != nil {
			slog.Error("failed to scan ad row", "error", err)
			return nil, err
		}
		if len(interests) > 0 {
			json.Unmarshal(interests, &ad.TargetingInterests)
		}
		ads = append(ads, ad)
	}
...
func (r *postgresAdsRepository) CreateCampaign(ctx context.Context, campaign *domain.AdCampaign) error {
	...
	_, err := r.client.Pool.Exec(ctx, query, campaign.ID, campaign.AdvertiserID, campaign.Name, campaign.BudgetTotal, campaign.BudgetRemaining, campaign.Status)
	if err != nil {
...
func (r *postgresAdsRepository) UpdateBudget(ctx context.Context, campaignID string, amount int32) error {
	...
	_, err := r.client.Pool.Exec(ctx, query, amount, campaignID)
	if err != nil {
...
func (r *postgresAdsRepository) CreateAdConversion(ctx context.Context, conversion *domain.AdConversion) error {
	...
	_, err := r.client.Pool.Exec(ctx, query, conversion.ID, conversion.ClickID, conversion.ConversionType, conversion.Value, conversion.Currency, conversion.ConversionTimestamp)
	if err != nil {
