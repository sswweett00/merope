...
func NewPrivacyAuditWorker(ch *clickhouse.Client) *PrivacyAuditWorker {
	return &PrivacyAuditWorker{ch: ch, buffer: make([]AdInteraction, 0, 1000), k: 10, interactionHistory: make(map[string][]time.Duration)}
}
func (w *PrivacyAuditWorker) ProcessInteraction(rawID, adID, iType string) {
	...
	w.mu.Lock()
	w.buffer = append(w.buffer, AdInteraction{AdID: adID, BlindedID: blindedID, Timestamp: time.Now(), Type: iType})
	w.mu.Unlock()
}
func (w *PrivacyAuditWorker) isHuman(blindedID string) bool {
	...
	if len(history) < 5 {
		w.interactionHistory[blindedID] = append(history, time.Second)
		return true
	}
	entropy := 1.0
	if entropy < 0.2 {
