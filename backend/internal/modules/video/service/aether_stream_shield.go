...
type AetherStreamShield interface {
	AuthorizeStreamAccess(context.Context, string, string, string) (string, error)
	IssueStreamDRMToken(context.Context, string, string, time.Duration) (string, error)
	TrackViewerSession(context.Context, string, string) error
	RevokeViewerSession(context.Context, string, string) error
}
...
func NewAetherStreamShield(bus events.Publisher, secretKey string) AetherStreamShield {
	...
	return &aetherStreamShield{bus: bus, secretKey: []byte(secretKey), activeViewers: make(map[string]map[string]time.Time)}
}
func (s *aetherStreamShield) AuthorizeStreamAccess(ctx context.Context, streamID, viewerID, tokenHex string) (string, error) {
	...
	if s.bus != nil {
		_ = s.bus.Publish(ctx, "aether.stream.authorized", events.Event{Type: "AETHER_STREAM_AUTHORIZED", Payload: map[string]interface{}{"stream_id": streamID, "viewer_id": viewerID, "watermark_code": watermarkCode, "authorized_at": time.Now()}})
	}
...
func (s *aetherStreamShield) IssueStreamDRMToken(ctx context.Context, streamID, viewerID string, ttl time.Duration) (string, error) {
	...
	return fmt.Sprintf("%s.%s", payload, hex.EncodeToString(h.Sum(nil))), nil
}
...
func (s *aetherStreamShield) RevokeViewerSession(ctx context.Context, streamID, viewerID string) error {
	...
	if s.bus != nil {
		_ = s.bus.Publish(ctx, "aether.stream.revoked", events.Event{Type: "AETHER_STREAM_REVOKED", Payload: map[string]interface{}{"stream_id": streamID, "viewer_id": viewerID}})
	}
	return nil
}
func (s *aetherStreamShield) generateWatermarkCode(streamID, viewerID string) string {
	day := time.Now().Unix() / 86400
	raw := fmt.Sprintf("%s#%s#%d", streamID, viewerID, day)
