...
func (h *IdentityHandler) Register(c *fiber.Ctx) error {
	type request struct {
		...
		Type        string `json:"type"` // "PERSONAL" or "CORPORATE"
	}
	...
	if strings.ToUpper(req.Type) == "CORPORATE" {
	...
	_ = security.StoreRefreshToken(c.Context(), h.rdb, user.ID, refreshToken, 30*24*time.Hour)
...
func (h *IdentityHandler) Login(c *fiber.Ctx) error {
	...
	user, token, mfaRequired, err := h.service.Login(
		c.Context(),
		req.Email,
		req.Password,
		req.Fingerprint,
...
func (h *IdentityHandler) VerifyMFA(c *fiber.Ctx) error {
	type request struct {
		UserID string `json:"user_id"` // Optional if already logged in partially
...
func (h *IdentityHandler) Logout(c *fiber.Ctx) error {
	...
	expiration := c.Locals("token_exp").(time.Time)
...
func (h *IdentityHandler) PasskeyLogin(c *fiber.Ctx) error {
	// Skeleton for WebAuthn/Passkey validation
	return c.JSON(fiber.Map{
		"message": "Passkey validation endpoint ready. Integration with WebAuthn library required.",
		"status":  "experimental",
	})
}
