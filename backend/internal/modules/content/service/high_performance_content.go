...
func (s *HighPerformanceContentService) BroadcastSignal(ctx context.Context, signal *domain.Signal, pool *domain.WavePoolData) (*domain.Signal, error) {
	...
	g.Go(func() error {
		...
		for _, m := range mentions {
			mEntities = append(mEntities, &domain.Mention{UserID: m, EntityType: "signal", EntityID: signal.ID})
		}
	...
	g.Go(func() error {
		return s.bus.Publish(gCtx, "content.signal.broadcasted", events.Event{Type: "SIGNAL_BROADCASTED", Payload: signal})
	})
...
func (s *HighPerformanceContentService) AmplifySignal(ctx context.Context, userID, signalID string, amplitude int) error {
	...
	if s.scyllaRepo != nil {
		if mirrorErr := s.scyllaRepo.AddResonance(ctx, userID, signalID, amplitude); mirrorErr != nil {
			log.Printf("[ScyllaDB] Failed to mirror resonance for signal %s: %v", signalID, mirrorErr)
		}
	}
	if pubErr := s.bus.Publish(ctx, "content.resonance.amplified", events.Event{Type: "SIGNAL_AMPLIFIED", Payload: map[string]interface{}{"user_id": userID, "signal_id": signalID, "amplitude": amplitude}}); pubErr != nil {
