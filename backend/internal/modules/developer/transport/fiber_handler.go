...
func(h *DeveloperHandler)CreateWebhook(c *fiber.Ctx)error{var req struct{Name,URL string;Events []string `json:"events"`};if err:=c.BodyParser(&req);err!=nil{return writeErr(c,err)};wh,err:=h.service.CreateWebhook(c.Context(),c.Params("app_id"),req.Name,req.URL,req.Events);if err!=nil{return writeErr(c,err)};return c.Status(fiber.StatusCreated).JSON(wh)}
...
func(h *DeveloperHandler)CreateBot(c *fiber.Ctx)error{var req struct{Name,Description string};if err:=c.BodyParser(&req);err!=nil{return writeErr(c,err)};bot,err:=h.service.CreateBot(c.Context(),c.Params("app_id"),req.Name,req.Description);if err!=nil{return writeErr(c,err)};return c.Status(fiber.StatusCreated).JSON(bot)}
...
func(_ *DeveloperHandler) normalize(_ string)string{return strings.TrimSpace("")}
