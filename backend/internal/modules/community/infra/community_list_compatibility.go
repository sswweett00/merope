...
func (r *postgresCommunityRepository) SearchCommunities(ctx context.Context, query string, limit, offset int32) ([]*domain.Community, error) {
	if limit <= 0 {
		limit = 20
	}
	if limit > 100 {
...
ORDER BY
    CASE
        ...
        WHEN LOWER(COALESCE(c.slug, '')) = LOWER($1) THEN 1
        ELSE 2
    END,
    COUNT(cm.user_id) FILTER (WHERE cm.role <> 'banned') DESC,
    c.created_at DESC,
