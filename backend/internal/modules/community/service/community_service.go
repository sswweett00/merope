...
func (s *communityService) ReplyToThread(ctx context.Context,threadID,authorID,content string)(*domain.ThreadReply,error){reply:=&domain.ThreadReply{ID:uuid.New().String(),ThreadID:threadID,AuthorID:authorID,Content:security.SanitizeHTML(content),CreatedAt:time.Now(),UpdatedAt:time.Now()};if err:=s.repo.CreateThreadReply(ctx,reply);err!=nil{return nil,err};return reply,nil}
func (s *communityService) PinThread(ctx context.Context,threadID string,pinned bool)error{return s.repo.PinThread(ctx,threadID,pinned)}
func (s *communityService) LockThread(ctx context.Context,threadID string,locked bool)error{return s.repo.LockThread(ctx,threadID,locked)}
func (s *communityService) PromoteMember(ctx context.Context,commID,userID,role string)error{return s.repo.UpdateMemberRole(ctx,commID,userID,role)}
func (s *communityService) DemoteMember(ctx context.Context,commID,userID,role string)error{return s.repo.UpdateMemberRole(ctx,commID,userID,role)}
func (s *communityService) BanMember(ctx context.Context,commID,userID,reason string,duration *time.Time)error{return s.repo.BanMember(ctx,commID,userID,reason,duration)}
func (s *communityService) UnbanMember(ctx context.Context,commID,userID string)error{return s.repo.UnbanMember(ctx,commID,userID)}
func (s *communityService) ReportContent(ctx context.Context,reporterID,contentType,contentID,reason string)error{return s.repo.ReportContent(ctx,reporterID,contentType,contentID,reason)}
func (s *communityService) ResolveReport(ctx context.Context,reportID,resolution string)error{return s.repo.ResolveReport(ctx,reportID,resolution)}
func (s *communityService) SetCommunityGuidelines(ctx context.Context,communityID string,guidelines []*domain.Guideline)error{return s.repo.UpdateGuidelines(ctx,communityID,guidelines)}
func (s *communityService) GetCommunityAnalytics(ctx context.Context,communityID,period string)(*domain.CommunityAnalytics,error){return s.repo.GetCommunityAnalytics(ctx,communityID,period)}
func (s *communityService) GetMemberActivity(ctx context.Context,communityID,userID,period string)(*domain.MemberActivity,error){return s.repo.GetMemberActivity(ctx,communityID,userID,period)}
func (s *communityService) SearchCommunities(ctx context.Context,query string,filters map[string]interface{})([]*domain.Community,error){limit:=int32(20);offset:=int32(0);if v,ok:=filters["limit"].(int32);ok{limit=v};if v,ok:=filters["offset"].(int32);ok{offset=v};return s.repo.SearchCommunities(ctx,query,limit,offset)}
func (s *communityService) GetTrendingCommunities(ctx context.Context,limit int32)([]*domain.Community,error){return s.repo.GetTrendingCommunities(ctx,limit)}
func (s *communityService) GetRecommendedCommunities(ctx context.Context,userID string,limit int32)([]*domain.Community,error){return s.repo.ListCommunities(ctx,userID,"",limit,0)}
