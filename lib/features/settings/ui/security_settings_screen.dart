...
class SecuritySettingsScreen extends ConsumerWidget {
      ...
      body: ListView(
        children: [
          SwitchListTile(
            ...
            subtitle: Text('Biyometrik (Yüz/Parmak İzi) ile giriş yapın',
          ...
          SwitchListTile(
            title: Text('İki Faktörlü Doğrulama (2FA)',
                style: TextStyle(color: tokens.textPrimary)),
            subtitle: Text('Hesabınızı ek bir güvenlik katmanıyla koruyun',
                style: TextStyle(color: tokens.textSecondary)),
          ...
          Padding(
            ...
            child: Text('Aktif Oturumlar',
                style: TextStyle(
                    fontWeight: FontWeight.bold, color: tokens.textPrimary)),
          ),
          _buildSessionTile('Android SDK 34 - Istanbul', '192.168.1.1',
              'Şu an aktif', true, tokens),
          _buildSessionTile('Web - Chrome - Ankara', '85.100.20.10',
              '2 saat önce', false, tokens),
          const Divider(),
          ListTile(
            leading: Icon(Icons.exit_to_app, color: tokens.dndStatus),
            title: Text('Tüm Oturumlardan Çıkış Yap',
  ...
  Widget _buildSessionTile(String device, String ip, String lastUsed,
      bool isCurrent, MeropeColorTokens tokens) {
    return ListTile(
      leading: Icon(Icons.devices, color: tokens.primary),
      title: Text(device, style: TextStyle(color: tokens.textPrimary)),
      subtitle: Text('$ip • $lastUsed',
          style: TextStyle(color: tokens.textSecondary)),
      trailing: isCurrent
