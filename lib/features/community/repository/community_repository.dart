...
class DriftCommunityRepository implements ICommunityRepository {
  ...
  Future<List<Community>> getCommunities({int limit = 20, int offset = 0}) async {
    final rows = await (_db.select(_db.subCommunities)..limit(limit, offset: offset)).get();

    return rows.map((row) => Community(
      id: row.id,
      ownerId: row.ownerId,
      name: row.name,
      slug: row.slug,
  ...
  Future<Community?> getCommunity(String communityId) async {
    final row = await (_db.select(_db.subCommunities)..where((t) => t.id.equals(communityId))).getSingleOrNull();
  ...
  Future<void> leaveCommunity(String communityId) async {
    await (_db.delete(_db.communityMembers)
      ..where((t) => t.communityId.equals(communityId) & t.userId.equals(_userId)))
      .go();
  }
  ...
  Future<List<CommunityEvent>> getCommunityEvents(String communityId, {int limit = 10, int offset = 0}) async {
    ...
    return rows.map((row) => CommunityEvent(
      ...
      status: EventStatus.values.firstWhere((e) => e.name == row.status, orElse: () => EventStatus.draft),
      createdAt: row.createdAt,
  ...
  Future<void> cancelEvent(String eventId) async {
      ...
      .write(db.CommunityEventsCompanion(status: Value(EventStatus.cancelled.name)));
  }

  @override
  Future<List<Collective>> getCollectives({int limit = 20, int offset = 0}) async {
    final rows = await (_db.select(_db.collectives)..limit(limit, offset: offset)).get();
    return rows.map((row) => Collective(
      id: row.id,
      name: row.name,
      slug: row.slug,
      description: row.description,
  ...
  Future<List<CollectiveThread>> getThreads(String collectiveId, {int limit = 20, int offset = 0}) async {
    final rows = await (_db.select(_db.collectiveThreads)
      ...
      ..limit(limit, offset: offset)).get();
    return rows.map((row) => CollectiveThread(
      id: row.id,
  ...
  Future<void> resonateThread(String threadId, int delta) async {
    final thread = await (_db.select(_db.collectiveThreads)..where((t) => t.id.equals(threadId))).getSingleOrNull();
    if (thread != null) {
      await (_db.update(_db.collectiveThreads)..where((t) => t.id.equals(threadId)))
        .write(db.CollectiveThreadsCompanion(resonance: Value(thread.resonance + delta)));
    }
  }
  ...
  Future<List<CommunityMember>> getCommunityMembers(String communityId, {int limit = 50, int offset = 0}) async {
     final rows = await (_db.select(_db.communityMembers)
       ..where((t) => t.communityId.equals(communityId))
       ..limit(limit, offset: offset)).get();
     return rows.map((row) => CommunityMember(
       id: row.id,
       communityId: row.communityId,
       userId: row.userId,
       username: 'User ${row.userId}',
       role: CommunityRole.values.firstWhere((e) => e.name == row.role, orElse: () => CommunityRole.member),
  ...
  Future<void> updateMemberRole(String communityId, String memberId, String role) async {
    await (_db.update(_db.communityMembers)..where((t) => t.communityId.equals(communityId) & t.id.equals(memberId)))
