...
class EscrowDetailsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<EscrowDetailsScreen> createState() =>
      _EscrowDetailsScreenState();
}
...
class _EscrowDetailsScreenState extends ConsumerState<EscrowDetailsScreen>
  ...
  void initState() {
    super.initState();
    _stageController =
        AnimationController(vsync: this, duration: MeropeTokens.durationSlow);
    _stageAnimation = Tween<double>(begin: 0, end: 1).animate(CurvedAnimation(
        parent: _stageController, curve: MeropeTokens.curveMeropeStandard));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title:
            Text("Secure Payment", style: TextStyle(color: tokens.textPrimary)),
        actions: [
          IconButton(
      ...
      body: escrowAsync.when(
        data: (escrow) {
          return RefreshIndicator(
            onRefresh: () async {
              await ref
                  .read(escrowDetailsProvider(widget.escrowId).notifier)
                  .build(widget.escrowId);
            },
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildDetailSection(
                      "Description", escrow.description, tokens),
                  ...
                  if (escrow.status == 'Held') ...[
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () => ref
                            .read(
                                escrowDetailsProvider(widget.escrowId).notifier)
                            .releasePayment(),
  ...
  Widget _buildStatusHeader(MeropeColorTokens tokens, MeropeEscrow escrow) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: (isHeld ? tokens.onlineStatus : tokens.secondary)
      ...
      child: Row(
        children: [
          AnimatedSwitcher(
            duration: MeropeTokens.durationNormal,
            child: Icon(isHeld ? Icons.lock_clock : Icons.check_circle,
                key: ValueKey(isHeld),
                color: isHeld ? tokens.onlineStatus : tokens.secondary,
                size: 32),
          ),
  ...
  Widget _buildAnimatedTimeline(
      EscrowStage activeStage, MeropeColorTokens tokens) {
        ...
        builder: (context, child) {
          return Opacity(
            ...
            child: Transform.translate(
                offset: Offset(0, 20 * (1 - _stageAnimation.value)),
                child: child),
          );
        },
        child: Container(
  ...
  Widget _buildNode(
      EscrowStage stage, bool isCompleted, MeropeColorTokens tokens) {
  ...
  Widget _buildCountdownTimer(
      MeropeColorTokens tokens, DateTime autoReleaseAt) {
          ...
          Expanded(
            child: Text(
              isExpired
                  ...
                  : 'Auto-release in ${_formatDuration(remaining)}',
  ...
  Widget _buildDetailSection(
      String label, String value, MeropeColorTokens tokens) {
    return Padding(
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: () async {
                      ...
                      await ref
                          .read(escrowDetailsProvider(widget.escrowId).notifier)
                          .openDispute(reasonController.text,
                    ...
                    style: ElevatedButton.styleFrom(
                        backgroundColor: tokens.dndStatus,
                        foregroundColor: Colors.white,
                        padding: const EdgeInsets.all(16)),
