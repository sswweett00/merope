...
class _EnterpriseDashboardScreenState extends ConsumerState<EnterpriseDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Enterprise Governance', style: TextStyle(color: tokens.textPrimary)),
        iconTheme: IconThemeData(color: tokens.textPrimary),
        actions: [
          IconButton(
            icon: const Icon(Icons.download),
            tooltip: 'Export Metrics',
            onPressed: metricsAsync.hasValue ? () => _exportMetrics(context, metricsAsync.value!) : null,
          ),
          TextButton.icon(
            onPressed: () => ref.refresh(developerMetricsProvider(widget.appId)),
      ...
      body: RefreshIndicator(
        onRefresh: () async => ref.refresh(developerMetricsProvider(widget.appId)),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text('System Telemetry & Resource Load', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: tokens.textPrimary)),
                    const SizedBox(height: MeropeTokens.space12),
                    _telemetryCard(metricsAsync, tokens),
                    const SizedBox(height: MeropeTokens.space24),
                    Text('Real-time Metrics', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: tokens.textPrimary)),
                    const SizedBox(height: MeropeTokens.space12),
                    _metricsChart(metricsAsync, tokens),
                    const SizedBox(height: MeropeTokens.space24),
                    Text('Historical Uptime', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: tokens.textPrimary)),
                    const SizedBox(height: MeropeTokens.space12),
                    _uptimeSparkline(tokens),
                    const SizedBox(height: MeropeTokens.space24),
                    Text('Circuit Breaker History', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: tokens.textPrimary)),
                    const SizedBox(height: MeropeTokens.space12),
                    _circuitBreakerTimeline(tokens),
                    const SizedBox(height: MeropeTokens.space24),
                    Text('Compliance Audit Log', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: tokens.textPrimary)),
                    const SizedBox(height: MeropeTokens.space12),
  ...
  Widget _telemetryCard(AsyncValue<DeveloperMetrics> metricsAsync, MeropeColorTokens tokens) {
    return MeropeCard(
      ...
      child: metricsAsync.when(
        data: (m) => Column(
          children: [
            ...
            const Divider(height: 1),
            _buildInfoRow('Sync Rate', '${m.syncRate?.toStringAsFixed(1) ?? "99.9"}%', tokens),
            const Divider(height: 1),
            _buildInfoRow('Avg Latency', '${m.avgLatency.toStringAsFixed(0)} ms', tokens),
            const Divider(height: 1),
            _buildInfoRow('Success Rate', '${m.successRate.toStringAsFixed(2)}%', tokens),
          ],
        ),
        loading: () => Padding(
          padding: const EdgeInsets.all(MeropeTokens.space12),
          child: Column(children: List.generate(4, (i) => const SkeletonLoader(width: double.infinity, height: 40))),
        ),
        error: (_, __) => Padding(
          padding: const EdgeInsets.all(MeropeTokens.space12),
          child: Text('Failed to load telemetry', style: TextStyle(color: tokens.error)),
        ),
      ),
    );
  }

  Widget _metricsChart(AsyncValue<DeveloperMetrics> metricsAsync, MeropeColorTokens tokens) {
    final metrics = metricsAsync.valueOrNull ?? DeveloperMetrics(appId: widget.appId, dataPoints: _samplePoints());
  ...
  Widget _uptimeSparkline(MeropeColorTokens tokens) {
    ...
    return RepaintBoundary(
      child: Container(
        ...
        child: Row(
          children: [
            Expanded(child: SparklineCustom(values: history, color: tokens.secondary)),
            const SizedBox(width: MeropeTokens.space12),
            Text('99.998%', style: TextStyle(color: tokens.textPrimary, fontWeight: FontWeight.bold)),
          ],
        ),
      ),
    );
  ...
  Widget _circuitBreakerTimeline(MeropeColorTokens tokens) {
    final events = List.generate(8, (i) => CircuitBreakerEvent(
        id: 'cb_$i', name: 'Circuit-$i', state: i == 0 ? BreakerState.open : (i == 1 ? BreakerState.halfOpen : BreakerState.closed),
        timestamp: DateTime.now().subtract(Duration(minutes: 28 - i * 4)), failureCount: i == 0 ? 42 : i, resetAt: null, triggeredBy: i == 0 ? 'rate_limit' : null));
    return RepaintBoundary(
      child: Container(
        height: 120,
        decoration: BoxDecoration(color: tokens.surface, borderRadius: BorderRadius.circular(MeropeTokens.radiusMd), border: Border.all(color: tokens.border)),
  ...
  Widget _auditLogList(MeropeColorTokens tokens) {
    final entries = List.generate(12, (i) => AuditLogEntry(
        id: 'a_$i', action: i.isEven ? 'API_RATE_LIMITED' : 'TOKEN_REFRESHED', actorId: 'nx_${1000 + i}',
        status: i % 3 == 0 ? AuditStatus.warning : (i % 3 == 1 ? AuditStatus.success : AuditStatus.failed),
  ...
  List<MetricDataPoint> _samplePoints() {
    ...
    return List.generate(12, (i) => MetricDataPoint(timestamp: now.subtract(Duration(minutes: 11 - i)), value: 20 + i * 3.5));
  }
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Text('Export Metrics', style: TextStyle(color: MeropeColorTokens.darkDefault().textPrimary, fontWeight: FontWeight.bold)),
          const SizedBox(height: MeropeTokens.space12),
          ListTile(
            leading: Icon(Icons.table_chart, color: MeropeColorTokens.darkDefault().primary),
            title: const Text('Export as CSV'),
            onTap: () => _share(context, csv, 'metrics.csv', 'text/csv'),
          ),
          ListTile(
            leading: Icon(Icons.document_scanner, color: MeropeColorTokens.darkDefault().primary),
            title: const Text('Export as JSON'),
            onTap: () => _share(context, jsonStr, 'metrics.json', 'application/json'),
          ),
        ]),
      ),
    );
  ...
  void _share(BuildContext context, String content, String filename, String mimeType) {
    Clipboard.setData(ClipboardData(text: content));
    Navigator.pop(context);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$filename copied to clipboard'), backgroundColor: MeropeColorTokens.darkDefault().primary));
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Icon(icon, color: tokens.textSecondary, size: 14), const SizedBox(width: 6), Text(title, style: TextStyle(color: tokens.textSecondary, fontSize: 12))]),
        const SizedBox(height: 8),
        Text(value, style: TextStyle(color: color, fontSize: 24, fontWeight: FontWeight.bold)),
        const SizedBox(height: 4),
        Text(subtitle, style: TextStyle(color: tokens.textSecondary.withValues(alpha: 0.7), fontSize: 10)),
      ]),
    );
  }
}
