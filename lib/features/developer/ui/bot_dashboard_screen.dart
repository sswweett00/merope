...
class _BotDashboardScreenState extends ConsumerState<BotDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title:
            Text('Developer Bots', style: TextStyle(color: tokens.textPrimary)),
        ...
        children: [
          ...
          Expanded(
            child: RefreshIndicator(
              onRefresh: () =>
                  ref.refresh(developerBotsProvider(widget.appId).future),
  ...
  Widget _buildBulkActions(MeropeColorTokens tokens) {
    return Container(
      ...
      padding: const EdgeInsets.symmetric(
          horizontal: MeropeTokens.space16, vertical: MeropeTokens.space8),
      child: Row(children: [
        ...
        TextButton.icon(
            ...
            icon: const Icon(Icons.power, size: 14),
  ...
  void _bulkToggle(bool enable) async {
    for (final id in _selected) {
      ...
      final bot = bots.firstWhere((b) => b.id == id,
          orElse: () => throw StateError('not found'));
      await ref
          .read(developerBotsProvider(widget.appId).notifier)
          .updateBot(bot.copyWith(isActive: enable));
    }
  ...
  void _confirmBulkDelete() {
    showDialog(
        ...
        builder: (context) => AlertDialog(
              ...
              content: Text(
                  'Delete ${_selected.length} bot(s)? This cannot be undone.'),
              actions: [
                TextButton(
  ...
  Widget _buildBotList(List<Bot> bots, MeropeColorTokens tokens) {
    ...
    if (filtered.isEmpty) {
      return Center(
          child: Text('No bots match your filter',
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, i) => _BotCard(
        ...
        onToggle: (v) => ref
            .read(developerBotsProvider(widget.appId).notifier)
            .updateBot(filtered[i].copyWith(isActive: v)),
      ),
    );
  }
  ...
  List<Bot> _applyFilter(List<Bot> bots) {
    ...
    var result =
        bots.where((b) => b.name.toLowerCase().contains(query)).toList();
    if (_filter == 'active') result = result.where((b) => b.isActive).toList();
    if (_filter == 'inactive')
      result = result.where((b) => !b.isActive).toList();
    return result;
  }
  ...
  void _toggleSelection(String botId) {
    setState(() {
      if (_selected.contains(botId))
        _selected.remove(botId);
      else
        _selected.add(botId);
    });
  ...
  Widget _buildError(Object error, MeropeColorTokens tokens) {
    return Center(
        child: Text('Error: $error', style: TextStyle(color: tokens.error)));
  }
}
...
class _BotCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final successRate = stats != null && stats.messagesSent > 0
        ? (100 -
                (stats.errorsOccurred / stats.messagesSent * 100).clamp(0, 100))
            .toStringAsFixed(1)
        : '100%';
          ...
          child:
            ...
            Row(children: [
              CircleAvatar(
                  ...
                  child: Icon(Icons.smart_toy, color: tokens.primary)),
              ...
              Expanded(
                child: Column(
                    ...
                    children: [
                      ...
                      Text('Client ID: ${bot.appId.substring(0, 8)}...',
                          style: TextStyle(
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
              ...
              _BotStat(
                  label: 'Errors',
                  value: '${stats?.errorsOccurred ?? 0}',
                  tokens: tokens),
              _BotStat(label: 'Success', value: successRate, tokens: tokens),
              _BotStat(
                  label: 'Lat',
                  value:
                      '${stats?.avgResponseTime.toStringAsFixed(0) ?? "0"}ms',
                  tokens: tokens),
            ]),
          ]),
        ),
    ...
    return Padding(
      ...
      child: Column(
          ...
          children: [
            ...
            _detailRow('Capabilities',
                bot.capabilities.map((c) => c.type.name).join(', '), tokens),
            ...
            _detailRow(
                'Commands Executed', '${stats?.commandsExecuted ?? 0}', tokens),
            _detailRow('Errors', '${stats?.errorsOccurred ?? 0}', tokens),
            _detailRow(
                'Avg Response',
                '${stats?.avgResponseTime.toStringAsFixed(1) ?? "0"} ms',
                tokens),
            _detailRow('Uptime', '${(stats?.uptime ?? 0).toStringAsFixed(1)}%',
                tokens),
          ]),
    );
  }
      ...
      content: Form(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          TextFormField(
            ...
            decoration: InputDecoration(
                labelText: 'Bot Name', hintText: 'e.g. MessageHandler'),
          ...
          onPressed: () {
            if (formKey.currentState?.validate() == true) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                  content: Text('Bot created — configure via API')));
