...
class _ApiKeysScreenState extends ConsumerState<ApiKeysScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () =>
            ref.refresh(developerApiKeysProvider(widget.appId).future),
  ...
  Widget _buildKeyList(List<ApiKey> keys, MeropeColorTokens tokens) {
    final filtered = keys
        .where((k) =>
            k.name.toLowerCase().contains(_searchController.text.toLowerCase()))
        .toList();
    if (filtered.isEmpty) {
      return Center(
      ...
      itemBuilder: (context, i) {
        ...
        return RepaintBoundary(
            child: _ApiKeyTile(
                apiKey: key,
                tokens: tokens,
                onRevoke: () => _confirmRevoke(key.id),
                onCopy: () => _copyKey(key)));
      },
    );
  }
  ...
  void _copyKey(ApiKey key) {
    if (key.rawKey != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Copied ${key.keyPrefix}…'),
          backgroundColor: MeropeColorTokens.darkDefault().primary));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Raw key not available (fetched from list)')));
    }
  ...
  void _confirmRevoke(String keyId) {
    showDialog(
        ...
        builder: (context) => AlertDialog(
              ...
              content: const Text(
                  'This action cannot be undone. The key will become invalid immediately.'),
              actions: [
                TextButton(
                ...
                ElevatedButton(
                    onPressed: () {
                      ...
                      ref
                          .read(developerApiKeysProvider(widget.appId).notifier)
                          .revokeApiKey(keyId);
                      MeropeHaptics.heavyImpact();
                    },
                    style: ElevatedButton.styleFrom(
                        backgroundColor: MeropeColorTokens.darkDefault().error),
      ...
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          ...
          TextField(
              ...
              decoration: InputDecoration(
                  labelText: 'Key Name', hintText: 'Production Server')),
          ...
          TextField(
              ...
              decoration: InputDecoration(labelText: 'TTL (days)'),
        ...
        actions: [
          ...
          ElevatedButton(
              onPressed: () {
                ...
                ref
                    .read(developerApiKeysProvider(widget.appId).notifier)
                    .createApiKey(widget.appId, name,
                        descController.text.trim(), scopes, ttl);
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('API key generated')));
                MeropeHaptics.lightImpact();
              },
...
class _ApiKeyTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final remaining = apiKey.expiresAt != null
        ? apiKey.expiresAt!.difference(DateTime.now()).inDays
        : null;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(
              child: Text(apiKey.maskedDisplay,
                  style: TextStyle(
          ...
          const SizedBox(width: 12),
          IconButton(
              icon: Icon(Icons.copy, color: tokens.textSecondary, size: 16),
              tooltip: 'Copy',
              onPressed: onCopy),
        ]),
        ...
        Wrap(
            spacing: 6,
            runSpacing: 4,
            children: apiKey.scopes
                .map((s) => Chip(
                      label: Text(s,
                          style:
                              TextStyle(color: tokens.onPrimary, fontSize: 10)),
        ...
        Row(children: [
          Icon(Icons.schedule,
              color: isExpired ? tokens.error : tokens.textSecondary, size: 14),
          ...
          Text(
            remaining != null
                ...
                : 'No expiry',
            style: TextStyle(
                color: isExpired ? tokens.error : tokens.textSecondary,
                fontSize: 12),
          ),
          const SizedBox(width: 16),
          Icon(Icons.bar_chart, color: tokens.textSecondary, size: 14),
          const SizedBox(width: 4),
          Text('$usageCount requests',
              style: TextStyle(color: tokens.textSecondary, fontSize: 12)),
        ]),
      ]),
    );
