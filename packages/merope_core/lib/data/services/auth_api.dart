...
class AuthApi {
  ...
  Future<ApiResult<AuthUser>> login(
    ...
    try {
      final response = await _dio.post('/api/v10/auth/login', data: {
        'email': email,
        'password': password,
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (token != null) {
          await _sessionStorage.saveSession(
              ...
              expiresAt: token.contains('.') ? _parseJwtExpiry(token) : null,
              user: userMap != null ? AuthUser.fromJson(userMap) : null);
        }
        if (userMap != null)
          return ApiResult.success(AuthUser.fromJson(userMap),
              statusCode: response.statusCode);
        return ApiResult.error('User data missing in response',
            statusCode: response.statusCode);
      }
      return ApiResult.error(
          response.data?['error']?.toString() ?? 'Login failed',
          statusCode: response.statusCode);
  ...
  Future<ApiResult<AuthUser>> register(
    ...
    try {
      final response = await _dio.post('/api/v10/auth/register', data: {
        'username': username,
        'email': email,
        'password': password,
  ...
  Future<ApiResult<void>> logout() async {
    try {
      final result = await _dio.post('/api/v10/auth/logout');
      if (result.statusCode == 200 || result.statusCode == 204) {
        await _sessionStorage.clearSession();
        return const ApiResult.success(null, statusCode: 200);
      }
      await _sessionStorage.clearSession();
      return ApiResult.error('Logout returned ${result.statusCode}',
