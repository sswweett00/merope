...
class AuthApi {
  ...
  Future<ApiResult<AuthUser>> login({
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (userMap != null) {
          return ApiResult.success(AuthUser.fromJson(userMap), statusCode: response.statusCode);
        }
  ...
  Future<ApiResult<void>> logout() async {
    try {
      ...
      await _sessionStorage.clearSession();
      return ApiResult.error('Logout returned ${result.statusCode}', statusCode: result.statusCode);
    } on DioException catch (e) {
  ...
  int? _parseJwtExpiry(String token) {
    try {
      ...
      if (exp != null) return DateTime.fromMillisecondsSinceEpoch(exp * 1000).millisecondsSinceEpoch;
    } catch (_) {}
    return null;
  }
}
